...
function pluginMutations (userState) {
    ...
    return {
        ...
        SET_SYNCCLAUSES: function (state, _a) {
            ...
            if (where && isArray$1(where))
                state._conf.sync.where = where;
            if (orderBy && isArray$1(orderBy))
...
function pluginActions (firestoreConfig) {
    ...
    return {
        ...
        deleteDoc: function (_a, payload) {
            ...
            // 0. payload correction (only arrays)
            var ids = !isArray$1(payload) ? [payload] : payload;
            // 1. Prepare for patching
            // 2. Push to syncStack
        ...
        insertDoc: function (_a, payload) {
            ...
            if (payload === void 0) { payload = []; }
            // 0. payload correction (only arrays)
            var docs = !isArray$1(payload) ? [payload] : payload;
            // 1. Prepare for patching
            var syncStack = getters.prepareForInsert(docs);
            // 2. Push to syncStack
            state._sync.syncStack.inserts = state._sync.syncStack.inserts.concat(syncStack);
            // 3. Create or refresh debounce & pass id to resolve
            var payloadToResolve = isArray$1(payload) ? payload.map(function (doc) { return doc.id; }) : payload.id;
        ...
        fetch: function (_a, parameters) {
            ...
            if (!isArray$1(where))
                where = [];
            if (!isArray$1(orderBy))
                orderBy = [];
            if (isArray$1(whereFilters) && whereFilters.length)
        ...
        fetchAndAdd: function (_a, parameters) {
            ...
            return dispatch('fetch', parameters).then(function (querySnapshot) {
                ...
                if (isFunction$1(querySnapshot.forEach)) {
        ...
        openDBChannel: function (_a, parameters) {
            ...
            if (getters.collectionMode) {
                getters.getWhereArrays().forEach(function (whereParams) {
                    // @ts-ignore
                    dbRef = where.apply(void 0, __spreadArrays([dbRef], whereParams));
                });
                if (state._conf.sync.orderBy.length) {
                    dbRef = orderBy.apply(void 0, __spreadArrays([dbRef], state._conf.sync.orderBy));
                }
            }
...
function vuexEasyFirestore(easyFirestoreModule, _a) {
    ...
    return function (store) {
        ...
        if (!isArray$1(easyFirestoreModule))
